...
from DenseBlock import DenseNet
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=1, init_features=16):
        ...
        self.dense1 = DenseNet(growth_rate=4, num_layers=4, num_init_features=features) # f * 2 = 32
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.encoder2 = UNet._block(features * 2, features * 4, name="enc2") # f * 4 = 64
        self.dense2 = DenseNet(growth_rate=8, num_layers=8, num_init_features=features * 4) # f * 8 = 128 
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.encoder3 = UNet._block(features * 8, features * 16, name="enc3") # f * 16 = 256
        self.dense3 = DenseNet(growth_rate=16, num_layers=16, num_init_features=features * 16) # f * 32 = 512
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)

        # bottleneck 
        self.bottleneck = UNet._block(features * 32, features * 32, name="bottleneck") # f * 32 = 512
        self.bottleneck_dense = DenseNet(growth_rate=16, num_layers=32, num_init_features=features * 32) # f * 64 = 1024
